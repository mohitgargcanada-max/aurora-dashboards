# AURORA-BPX Production Addendum v1.0
## BasePivot, RMVP, Darvas, Auto-AVWAP & Renderer Integration

**Status:** Production-grade reference record — merged into AURORA v2.18.3 consolidated master  
**Record purpose:** Preserve the standalone BPX logic even after merger into the master specification.  
**Scope:** EOD scanner only. Intraday execution remains future scope.  
...
BPX is the **structure engine**. It answers: where is the actionable pivot, where is the early pivot, where is support, where is the danger level, and what should the renderer draw?

BPX must remain separate from VE2 and PBX at the conceptual level:

```text
BPX = WHERE is the structure?
VE2 = Is volume/fuel confirming?
PBX = Is this pullback worth buying?
AXM = Is it too late / too extended?
```

### Non-Negotiable Production Locks

```text
BPX does not create final buckets.
BPX does not place trades.
BPX does not replace RMV math.
BPX does not replace VE2 volume logic.
BPX does not override market permission, Stage 4, liquidity, PX, or AURORA-X.
BPX diagnostic labels must map into the locked AURORA final buckets only.
```
...
```text
[ ] Existing base detectors still produce base_type/base_start/base_end/base_high/base_low.
[ ] RMV pivot math remains owned by the RMV engine.
[ ] BPX reads RMV outputs and adds base-aware context only.
[ ] VE2 outputs are consumed for breakout confirmation; BPX does not recalculate volume signatures.
[ ] All BPX fields are added to renderer payload.
[ ] No BPX diagnostic label appears in final_bucket.
[ ] Tests cover failed probes, weak breakout progress, pivot merge, support flip, and Darvas breakdown.
```

# Production Self-Test Cases

```text
1. A 15+ day base with 3 aligned highs creates BASEPIVOT_QUALITY_A.
2. A 5-14 day shelf creates SHORT_FLAG_PIVOT or RMVP_ONLY, not Structural BasePivot.
3. A wick above pivot with weak close does not move the pivot higher.
4. A breakout that advances less than 2 ADR and falls back keeps the base active.
5. RMVP_LOW_CHEAT below BasePivot cannot become TRADE_READY without VE2 and market permission.
6. DARVAS_BREAKOUT_UNCONFIRMED remains diagnostic if VE2 confirmation is missing.
7. Danger level breach plus VE2 distribution maps to REPAIR_WATCH or AVOID_FRESH_LONG only.
```
