...
const allCandidateRows = (state.all_candidates || []).map(c => `<tr data-route="${esc(c.route)}" data-search="${esc(`${c.ticker} ${c.route} ${c.bucket} ${c.gics_sector || ""} ${(c.scans || []).join(" ")} ${(c.failed_gates || []).join(" ")} ${c.user_note || ""}`.toLowerCase())}"><td><strong>${esc(c.ticker)}</strong><small>${esc(c.exchange)}</small></td><td>${esc(c.gics_sector)}<small>${esc(c.main_industry || c.classification_status)}</small></td><td>${badge(c.route)}</td><td>${badge(c.bucket)}</td><td>${money(c.price)}</td><td>${esc(c.stage)}</td><td>${esc(c.rs_trifecta)} / ${esc(c.rs21)}</td><td>${fmt(c.rmv15)}</td><td>${fmt(c.entry_risk_pct ?? c.risk_pct)}%</td><td>${fmt(c.thesis_risk_pct)}%</td><td>${fmt(c.axm_atr)} ATR</td><td>${esc(c.pattern_proxy)}</td><td>${esc((c.scans || []).join(", ") || "NONE")}</td><td>${esc((c.failed_gates || []).join(", ") || "NONE")}</td><td>${esc(c.next_condition)}</td><td>${esc(c.user_note || c.pattern_note)}</td></tr>`).join("");
