...
export function serializeUsDashboardCandidate(row = {}, rank = null) {
  return {
    ...
    gics_sector_name: pick(row, ["gics_sector_name", "gics_sector"]),
    gics_industry_group_name: pick(row, ["gics_industry_group_name"]),
    gics_industry_name: pick(row, ["gics_industry_name", "main_industry"]),
    gics_sub_industry_name: pick(row, ["gics_sub_industry_name", "sub_industry"]),
    aurora_theme: pick(row, ["aurora_theme", "theme_primary", "theme"]),
    aurora_subtheme: pick(row, ["aurora_subtheme", "sub_industry"]),
    market_native_sector: pick(row, ["market_native_sector", "sector", "gics_sector"]),
    market_native_industry: pick(row, ["market_native_industry", "main_industry"]),
    market_native_subindustry: pick(row, ["market_native_subindustry", "sub_industry"]),
    classification_confidence: pick(row, ["classification_confidence", "gics_classification_confidence", "theme_confidence"]),
    ...
    scan_memberships: pick(row, ["scan_memberships"]) ?? [],
    current_gate: pick(row, ["current_gate"]),
    radar_reason: pick(row, ["radar_reason"]),
    strong_rs_retention_status: pick(row, ["strong_rs_retention_status"]),
    retention_reason: pick(row, ["retention_reason"]),
    myh_status: pick(row, ["myh_status"]),
    myh_approaching_status: pick(row, ["myh_approaching_status"]),
    myh_target_level: pick(row, ["myh_target_level", "myh_level"]),
    myh_gap_pct: pick(row, ["myh_gap_pct"]),
    myh_lookback_label: pick(row, ["myh_lookback_label", "myh_label"]),
    myh_history_confidence: pick(row, ["myh_history_confidence"]),
    myh_next_condition: pick(row, ["myh_next_condition"]),
    myh_breakout_level: pick(row, ["myh_breakout_level"]),
    bars_since_myh_breakout: pick(row, ["bars_since_myh_breakout"]),
    myh_retest_anchor: pick(row, ["myh_retest_anchor"]),
    myh_retest_distance_pct: pick(row, ["myh_retest_distance_pct"]),
    rs_trifecta_diagnostic_note: pick(row, ["rs_trifecta_diagnostic_note"]),
...
export function buildUsDashboardJsonExport({ latest = {}, scan = {}, generatedAt } = {}) {
  ...
  const scanPayload = {
    ...
    industry_group_rrg: cloneJson(sections.industry_group_rrg || []),
    industry_rrg: cloneJson(sections.industry_rrg || []),
    sub_industry_rrg: cloneJson(sections.sub_industry_rrg || []),
    aurora_radar_universe: serializeList(sections.aurora_radar_universe),
    strong_rs_retention: serializeList(sections.strong_rs_retention),
    soft_rs_reject_recovered: serializeList(sections.soft_rs_reject_recovered),
    myh_approaching: serializeList(sections.myh_approaching),
    myh_breakout_retest: serializeList(sections.myh_breakout_retest),
  ...
  for (const row of [
    ...
    ...scanPayload.aurora_radar_universe,
    ...scanPayload.strong_rs_retention,
    ...scanPayload.soft_rs_reject_recovered,
    ...scanPayload.myh_approaching,
    ...scanPayload.myh_breakout_retest,
