...
export function buildCanadaFeatureMatrix({ universe, stockRecords, benchmarkRecord, expectedSession }) {
  ...
  for (const item of universe) {
    ...
    const rmv5 = rmv(bars, 5); const rmv15 = rmv(bars, 15); const rmv25 = rmv(bars, 25);
    ...
    const entryStop = Math.min(stopAnchor * 0.995, entryRef - 0.5 * atr14);
    ...
    const row = {
      symbol: item.symbol,
      name: item.name,
      exchange: item.exchange || "TSX",
      provider: record.provider,
      data_as_of: record.data_as_of,
      price: round(price),
      change_1d_pct: round(priceChangePct(bars, 1) * 100),
      raw_rs: rawRs,
      rs21_state: rsState,
      rs_slope_5d: round(rsSlope5),
      rsnh: rsnh63 || rsnh252,
      rsnh63,
      rsnh252,
      rs_trifecta,
      rrg,
      rmv5: round(rmv5), rmv15: round(rmv15), rmv25: round(rmv25), rmv_label: rmvLabel(rmv15), compression,
      stage_label: stageLabel(bars),
      trigger_price: round(trigger), trigger_gap_pct: round(trigger_gap_pct),
      entry_reference: round(entryRef), entry_stop: round(entryStop), thesis_stop: round(thesisStop), entry_risk_pct: round(entryRiskPct), thesis_risk_pct: round((entryRef - thesisStop) / entryRef * 100),
      axm, ve2, ...bpx,
      pbx_label,
      theme: theme.theme, theme_confidence: theme.confidence,
      addv20: round(addv20, 0), avg_volume20: round(avgVolume20, 0), liquidity_label: liquidity,
      leadership_score: 0, tactical_score: 0, aurora_sig_score: 0,
      caution: "none from calculated technical fields",
      next_condition: "Needs next completed-session trigger acceptance with VE2 confirmation and valid market permission."
    };
    rows.push(row);
  }
...
function renderTable(title, id, rows, note = "") {
  ...
  return `<h2 id="${id}">${escapeHtml(title)}</h2>${note ? `<p class="notice">${escapeHtml(note)}</p>` : ""}<div class="table-wrap"><table><thead><tr>${REQUIRED_CANDIDATE_COLUMNS.map(c => `<th>${escapeHtml(c)}</th>`).join("")}</tr></thead><tbody>${body}</tbody></table></div>`;
}

function stockThemeLeadership(sections) {
  const counts = new Map();
  for (const [name, rows, weight] of sections) {
    for (const row of rows) {
...
export function renderCanadaDashboard(model) {
  const marketPermission = model.indexAudit.status === "INDEX_FRESHNESS_OK" && model.coverage.status === "COVERAGE_OK" ? "TRADE_ALLOWED_OR_SELECTIVE_ONLY" : "MARKET_STATE_UNKNOWN";
  ...
  const rejectedRows = model.rejected.map((r, i) => `<tr><td>${i + 1}</td><td>${escapeHtml(r.symbol)}</td><td>${escapeHtml(r.rejection_reason)}</td><td>Retry official/free route first; EODHD Canada fallback is NOT_IMPLEMENTED_NOT_TESTED.</td></tr>`).join("");
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>AURORA Canada Unified Dashboard</title><style>:root{--ink:#17201c;--muted:#66716b;--paper:#f7f8f5;--panel:#fff;--line:#d9ddd7;--green:#146b45;--greenbg:#e8f4ed;--amber:#895b00;--amberbg:#fff3d4;--red:#9b2f2f;--redbg:#fae9e7;--blue:#195a78;--bluebg:#e6f2f7}*{box-sizing:border-box}body{margin:0;background:var(--paper);color:var(--ink);font:14px/1.45 Inter,Arial,sans-serif}header.hero{background:#143e55;color:white;padding:22px 28px;border-bottom:4px solid #d8ad42}.hero h1{margin:0;font-size:26px}.hero p{margin:5px 0 0;color:#deebe3}.nav{display:flex;gap:8px;flex-wrap:wrap;margin-top:14px}.nav a{color:white;text-decoration:none;border:1px solid #7897a5;padding:6px 10px;border-radius:4px}.wrap{padding:20px 28px 42px}.summary{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:10px}.metric{background:var(--panel);border:1px solid var(--line);border-radius:6px;padding:12px}.metric b{display:block;font-size:17px;margin-top:4px}.metric small,td small{display:block;color:var(--muted);margin-top:3px}h2{font-size:19px;margin:28px 0 10px}.notice{background:var(--amberbg);border-left:4px solid #c38b16;padding:10px 12px;border-radius:4px}.table-wrap{overflow:auto;border:1px solid var(--line);background:white;border-radius:6px}table{border-collapse:collapse;width:100%;min-width:1560px}th,td{text-align:left;vertical-align:top;padding:8px 9px;border-bottom:1px solid var(--line);font-size:12px}th{background:#edf1ec;position:sticky;top:0;z-index:1;white-space:nowrap}.status{display:inline-block;padding:3px 6px;border-radius:4px;font-size:11px;font-weight:700;background:var(--bluebg);color:var(--blue);white-space:nowrap}.TRIGGER_READY,.TRADE_READY{color:var(--green);background:var(--greenbg)}.NO_CHASE,.AVOID_FRESH_LONG{color:var(--red);background:var(--redbg)}.foot{color:var(--muted);margin-top:18px}@media(max-width:900px){.summary{grid-template-columns:1fr 1fr}.wrap{padding:16px}header.hero{padding:18px}table{min-width:1280px}}</style></head><body><header class="hero"><h1>AURORA Canada Unified Dashboard</h1><p>Canada-only EOD scan · Completed session ${escapeHtml(model.expectedSession)} · Yahoo free-first route · CAD</p><nav class="nav"><a href="#market">Market</a><a href="#guide">Column Guide</a><a href="#weekly">Weekly Universe</a><a href="#focus">Focus</a><a href="#top">Daily Top</a><a href="#rsle">RSLE Top 20</a><a href="#developing">Developing</a><a href="#overlay">Canada Overlay</a><a href="#rrg">RRG</a><a href="#themes">Theme Leadership</a><a href="#rshigh">Near RS High</a><a href="#pullbacks">PBX Pullback</a><a href="#basepivots">BasePivot</a><a href="#rmvp">RMVP</a><a href="#ve2">VE2</a><a href="#compression">Compression</a><a href="#risk">No-Chase</a><a href="#rejected">Rejected</a><a href="#provenance">Provenance</a></nav></header><main class="wrap"><section class="summary"><div class="metric">Run state<b>FULL_LOCAL_SCAN</b><small>free-first, no default EODHD</small></div><div class="metric">Session<b>${escapeHtml(model.expectedSession)}</b><small>latest completed Canadian session</small></div><div class="metric">Index audit<b>${escapeHtml(model.indexAudit.status)}</b><small>${model.indexAudit.valid_symbols}/${model.indexAudit.checked_symbols} valid</small></div><div class="metric">Coverage<b>${num(model.coverage.coverage_pct)}%</b><small>${model.coverage.current_symbols}/${model.coverage.loaded_symbols} current</small></div><div class="metric">Market Permission<b>${escapeHtml(marketPermission)}</b><small>EOD only</small></div></section><h2 id="market">Market Summary Strength Stack</h2><p class="notice">Benchmark: ${CANADA_PROFILE.benchmark_primary}; growth proxy ${CANADA_PROFILE.benchmark_growth}; breadth proxy ${CANADA_PROFILE.benchmark_breadth}. Market permission is blocked if benchmark or sector/index data is stale.</p><h2 id="guide">Column Guide</h2><div class="table-wrap"><table><thead><tr><th>Column</th><th>Meaning</th></tr></thead><tbody>${REQUIRED_CANDIDATE_COLUMNS.map(c => `<tr><td><strong>${escapeHtml(c)}</strong></td><td>Canada-specific AURORA field preserved from India/US dashboard format.</td></tr>`).join("")}</tbody></table></div>${renderTable("WEEKLY_UNIVERSE", "weekly", model.weeklyUniverse, "Rolling 15-20 Canada candidates. No forced padding.")}${renderTable("WEEKLY_FOCUS", "focus", model.weeklyFocus, "Execution funnel from Weekly Universe.")}${renderTable("DAILY_TOP_1_4 Conditional Trade Plans", "top", model.dailyTop, "Maximum four; never forced.")}${renderTable("AURORA-RSLE Top 20", "rsle", model.rsleTop20, "Independent Canada RS leadership-entry list.")}${renderTable("Developing Watchlist Next 20", "developing", model.developing, "Emerging leaders awaiting setup, confirmation or tighter risk.")}<h2 id="overlay">Canada Small/Microcap / Exchange Overlay</h2><p class="notice">Initial production lane is TSX liquid universe. TSXV/CSE/NEO symbols must pass explicit suffix validation before inclusion; unmapped or unsupported suffixes go to DATA_REPAIR.</p><h2 id="rrg">Sector and Theme RRG</h2><p class="notice">Sector RRG uses validated Canadian proxies: XFN.TO, XEG.TO, XIT.TO, XMA.TO, ZIN.TO, ZUT.TO, XRE.TO versus ^GSPTSE.</p><h2 id="themes">Stock Theme Leadership</h2><div class="table-wrap"><table><thead><tr><th>Rank</th><th>Theme</th><th>Weekly</th><th>Daily Top</th><th>RSLE</th><th>Developing</th><th>Weighted Presence</th><th>Symbols</th></tr></thead><tbody>${themeRows}</tbody></table></div><h2 id="rrglegend">RRG Quadrant Map</h2><p class="notice">LEADING = ratio >=100 and momentum >=100; IMPROVING = ratio <100 and momentum >=100; WEAKENING = ratio >=100 and momentum <100; LAGGING = both below 100.</p>${renderTable("Near RS High", "rshigh", model.nearRsHigh)}${renderTable("PBX Pullback", "pullbacks", model.pullbacks)}${renderTable("BasePivot / Patterns", "basepivots", model.basepivots)}${renderTable("RMVP / Early Entry", "rmvp", model.rmvp)}${renderTable("VE2 Volume Signature", "ve2", model.ve2)}${renderTable("Compression", "compression", model.compression)}${renderTable("No-Chase / Risk", "risk", model.noChase)}<h2 id="rejected">Rejected / Data Repair Routes</h2><div class="table-wrap"><table><thead><tr><th>Rank</th><th>Symbol</th><th>Reason</th><th>Next Route</th></tr></thead><tbody>${rejectedRows || `<tr><td colspan="4">No rejected rows.</td></tr>`}</tbody></table></div><h2 id="provenance">Provenance</h2><div class="table-wrap"><table><thead><tr><th>Field</th><th>Value</th></tr></thead><tbody><tr><td>provider_route</td><td>${escapeHtml(JSON.stringify(model.indexAudit.provider_route))}</td></tr><tr><td>eodhd_canada_fallback_status</td><td>EODHD_FALLBACK_NOT_IMPLEMENTED_NOT_TESTED</td></tr><tr><td>expected_completed_session</td><td>${escapeHtml(model.expectedSession)}</td></tr><tr><td>latest_index_data_as_of</td><td>${escapeHtml(model.indexAudit.present_symbols.map(x => `${x.symbol}:${x.data_as_of}`).join(", "))}</td></tr><tr><td>symbols_loaded</td><td>${model.coverage.loaded_symbols}</td></tr><tr><td>valid_symbols</td><td>${model.coverage.valid_history_symbols}</td></tr><tr><td>feature_matrix_count</td><td>${model.rows.length}</td></tr><tr><td>scanned_candidates</td><td>${model.rows.length}</td></tr><tr><td>rejected_count</td><td>${model.rejected.length}</td></tr></tbody></table></div><p class="foot">AURORA Canada remains EOD-only. RS means benchmark-relative strength, never RSI. No intraday order logic or live automation.</p></main></body></html>`;
}
